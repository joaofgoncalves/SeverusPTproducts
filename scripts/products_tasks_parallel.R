...
## --------------------------------------------------------------------------- ##
## SeverusPT products processing main pipeline ----
## --------------------------------------------------------------------------- ##
...
repeat{
  ...
  # Stop looping through tasks in the table
  if(tidx > numTasks){
    break
  }
  ...
  # If the task as a whole finished or parts of it were started then just skip it
  # and proceed to the next one
  #
  # This strategy does not allow to recover tasks!
  #
  if(spt_get_taskStatus(task, "main") == "COMPLETED"){
    next
  }
  if(spt_get_taskStatus(task, "gee") != "GEE NOT COMPLETED"){
    next
  }
  if(spt_get_taskStatus(task, "post") != "POST NOT COMPLETED"){
    next
  }
  if(spt_get_taskStatus(task, "close") != "CLOSING NOT COMPLETED"){
    next
  }
  ...
  if(task$metadataTaskStatus != "METADATA COMPLETED"){
    ...
    # This table is for the secondary CRS in WGS 1984/UTM 29N (EPSG: 32629)
    #
    ...
    #
    meta_tm06 <- spt_fill_meta_template(
      metaTemplate = meta,
      metaList = list(
        #CoordRefSystem = "Secondary CRS: WGS 1984/UTM 29N (EPSG: 32629)"
        CoordRefSystem = "Primary CRS: ETRS1989/PTTM06 (EPSG: 3763)"
    ...
    out <- try({
      ...
      # Metadata for secondary CRS WGS1984 / UTM 29N
      # This is the CRS used for GEE calculations
      ...
      # CRS used in post-processing
      spt_export_to_md(meta_tm06, paste0(spt_replace_crs_code(outFilePath),".meta.txt"))
      write.csv(meta_tm06, paste0(spt_replace_crs_code(outFilePath),".meta.csv"),
                row.names = FALSE, quote = TRUE)
      spt_export_meta_to_json(meta_tm06, paste0(spt_replace_crs_code(outFilePath),".meta.json"))
